...
import { getS3PackageDirectory } from '../../lib/projectParser';
...
const _getS3PackageDirectory = async id => {
  const units = await prisma.package( { id } ).documents().$fragment( PACKAGE_DOCUMENT_FILES );
  return getS3PackageDirectory( units );
};
...
export const publishDelete = async id => {
  ...
  const channel = await createChannel();
  const projectDirectory = await _getS3PackageDirectory( id );
